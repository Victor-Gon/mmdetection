...
class WaymoMetric(BaseMetric):
    ...
    def xyxy2cxcywh(self, bbox: np.ndarray) -> list:
        _bbox = bbox.tolist()
        return [
            (_bbox[0] + _bbox[2]) / 2,
            (_bbox[1] + _bbox[3]) / 2,
    ...
    def results2dicts(self, results: Sequence[dict],
        ...
        for idx, result in enumerate(results):
            ...
            for i, label in enumerate(labels):
                ...
                data['score'] = scores[i]
                data['type'] = self.dataset_meta['class_type_to_submit'][class_name]
                data['id'] = f'{idx}_{label}_{i}'  # dummy tracking id
                bbox_dict_results.append(data)
        
        result_files = dict()
        result_files['bbox'] = f'{outfile_prefix}.bbox.pkl'
        dump(bbox_dict_results, result_files['bbox'])
    ...
    def compute_metrics(self, results: list, time_of_day:str=None) -> Dict[str, float]:
        ...
        # TODO: Waymo format
        self.results2dicts(preds, outfile_prefix)
        # self.results2proto(preds, outfile_prefix)
        ...
        for metric in self.metrics:
            ...
            if time_of_day:
                filt_imgs = [k for k, v in self._coco_api.imgs.items() if v['time_of_day'] == time_of_day]
                self._coco_api.imgToAnns = {
                    imgId: anns
                    for (imgId, anns) in self._coco_api.imgToAnns.items() if imgId in filt_imgs
                }
                anns = {k: v for (k, v) in self._coco_api.anns.items() if v['image_id'] in filt_imgs}
                self._coco_api.anns = anns


                coco_dt.imgToAnns = {
                    imgId: anns
                    for (imgId, anns) in coco_dt.imgToAnns.items() if imgId in filt_imgs
                }
                #
                anns = {k: v for (k, v) in coco_dt.anns.items() if v['image_id'] in filt_imgs}
                coco_dt.anns = anns
            ...
            if metric == 'proposal':
                ...
                if self.classwise:  # Compute per-category AP
                    ...
                    for idx, cat_id in enumerate(self.cat_ids):
                        ...
                        iou = np.where(
                                self.dataset_meta['classwise_iou'][nm["name"]] == coco_eval.params.iouThrs)[0]
                        ...
                        eval_results[f'{nm["name"]}_L2'] = round(ap, 4)
                    ...
                    headers = [
                        'category', 'mAP Waymo L2', 'mAP_50', 'mAP_75', 'mAP_s',
                        'mAP_m', 'mAP_l'
                    ]
                    ...
                    for category, category_iou in self.dataset_meta['classwise_iou'].items():
