...
def split_coco(data_root, out_dir, percent, fold):
    ...
    np.random.seed(fold)
    ann_file = osp.join(data_root, 'annotations/instances_train2020.json')
    anns = load(ann_file)
    ...
    labeled_name = f'instances_train2020.{fold}@{percent}'
    unlabeled_name = f'instances_train2020.{fold}@{percent}-unlabeled'
